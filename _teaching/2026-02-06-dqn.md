...
- The neural network of QR-DQN has outputs \\(N \times card(\mathcal{A})\\) outputs: \\(N\\) quantiles per possible action \\(a\\): \\((q_{\tau_0}(s, a; \theta), q_{\tau_1}(s, a; \theta), ..., q_{\tau_i}(s, a; \theta), ..., q_{\tau_{N-1}}(s, a; \theta))\\), whereas in DQN the neural network outputs \\(card(\mathcal{A})\\) values corresponding to the number of possible actions. 
...
- [xpag](https://github.com/perrin-isir/xpag/) (Buffer and Sampler classes, see [CREDITS.md](https://github.com/charlypg/DQN-practical/blob/main/CREDITS.md) for more information)
